...
*   (選用) Hugging Face Token，若要啟用講者辨識 (Pyannote) 功能。
...
3.  **Hugging Face Token (若要使用講者分辨功能)**
    如果要啟用講者辨識：
    1. 前往 Hugging Face 註冊帳號並建立 Access Token。
    2. 必須在 [speaker-diarization-3.1](https://huggingface.co/pyannote/speaker-diarization-3.1) 和 [segmentation-3.0](https://huggingface.co/pyannote/segmentation-3.0) 頁面點擊同意授權。
    3. 在本地執行 `gpu-worker` 時，需在終端機設定環境變數：`export HF_TOKEN="your_hf_token"`。
...
設定您的 Hugging Face Token (選用，用於講者辨識)：
```bash
...
| `ModuleNotFoundError` 或 Buildpacks 錯誤 | Dockerfile 被忽略 | 確認檔名為 `Dockerfile`，部署加上 `--clear-base-image` |
