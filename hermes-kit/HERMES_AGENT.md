...
### 每日檢查（定點觸發，非高頻輪詢）
每天固定 8 次 `trigger`：
**07:30、08:30、09:30、12:30、13:30、14:30、19:00、21:30**。
（19:00 與 21:30 兩次是為了撿下午場會後上架的 VOD，讓字幕當天寄出、
不積到隔天；漏網的仍由隔日早上檢查兜底——VOD 掃描範圍是今天+昨天。）
...
- （若管理者日後要求「會議當天即時拿到字幕」，才改回開會日 09:30–19:00
  每 5 分鐘輪詢的模式。）
- `trigger` 回傳的解讀（重要）：
  - `now_taiwan` 欄位是後端的台灣時間——**以它為準對時**，回答任何
    時間/進度問題前先看它，不要依賴自己的記憶或歷史紀錄。
  - `vods_queued` 裡的 `last_failure` 欄位：若存在，代表這筆 VOD 上次
    處理失敗及原因（例如「官方尚未上架 HLS（YouTube URL）」＝議會還沒
